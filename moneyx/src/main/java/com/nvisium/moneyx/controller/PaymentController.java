...
public class PaymentController {
	...
	public String listReceivedPayments(@PathVariable Long id, Model model) {

		List<Payment> payments = 
				paymentService.getReceivedPayments(userService.loadUserById(id));
		
		MoneyxLogger.log("Session-"+ RequestContextHolder.getRequestAttributes().getSessionId() +":/payment/list-received/"+id);
	...
	public String listSentPayments(@PathVariable Long id, Model model) {

		List<Payment> payments = 
				paymentService.getReceivedPayments(userService.loadUserById(id));
		
		MoneyxLogger.log("Session-"+ RequestContextHolder.getRequestAttributes().getSessionId() +":/payment/list-sent/"+id);
	...
	public String balance(@RequestParam(value = "amount", required = false) String amount,
			@RequestParam(value = "creditcard", required = false) String creditcard, RedirectAttributes redirectAttrs,
			Model model) {

		if (creditcard == null || amount == null) {
			model.addAttribute("user", security.getSecurityContext().getUser());
			MoneyxLogger.log("Session-"+ RequestContextHolder.getRequestAttributes().getSessionId() +":/payment/balance/");
		...
		em.createNativeQuery(
				"Update Users u set balance = balance + :amount where id = " + security.getCurrentUserId())
		...
		MoneyxLogger.log("Session-"+ RequestContextHolder.getRequestAttributes().getSessionId() + ":/payment/balance/:"+creditcard+":"+amount);
	...
	public String makePayment(@RequestParam(value = "event", required = false) Long eventId,
		...
		if (eventId == null && amount == null) {
			for (EventMembership m : memberships) {
				...
				users.put(m.getEventId(),
						userService.loadUserById(eventService.getEventById(m.getEventId()).getOwner()));
				eventMemberships.put(m.getEventId(), m);
			}
			model.addAttribute("users", users);
			model.addAttribute("events", events);
			model.addAttribute("memberships", eventMemberships);
			MoneyxLogger.log("Session-"+ RequestContextHolder.getRequestAttributes().getSessionId() +":/payment/make-payment");
			...
			model.addAttribute("events", events);
			model.addAttribute("memberships", eventMemberships);
			MoneyxLogger.log("Session-"+ RequestContextHolder.getRequestAttributes().getSessionId() +":/payment/make-payment:"+eventId+":"+amount);
			return "payment/make-payment";
		}
